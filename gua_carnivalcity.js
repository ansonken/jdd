...
function updateShareCodesCDN(url = 'https://cdn.jsdelivr.net/gh/11111120/updateTeam@master/shareCodes/jd_cityShareCodes.json') {
  return new Promise(resolve => {
...
function readShareCode() {
  ...
  return new Promise(async resolve => {
    $.get({url: `https://jd.11111118/info_carnivalcity`, 'timeout': 20000}, (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
          console.log(`${$.name} API请求失败，请检查网路重试`)
