...
!(async () => {
    ...
    let res = [];
    try{res = await getAuthorShareCode('https://raw.githubusercontent.com/123/share_code/main/vivo.json');}catch (e) {}
    if(!res){
        try{res = await getAuthorShareCode('https://cdn.jsdelivr.net/gh/123/share_code@main/vivo.json');}catch (e) {}
        if(!res){res = [];}
    }
    if(res.length > 0){
