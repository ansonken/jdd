...
cron 23 7,9 * * * https://raw.githubusercontent.com/333333/jd/main/scripts/jd_xsljd.js
* */
const $ = new Env('限时抢京豆');
...
!(async () => {
    ...
    let res = [];
    try{res = await getAuthorShareCode('https://raw.githubusercontent.com/11111110/share_code/main/shop.json');}catch (e) {}
    if(!res){
        try{res = await getAuthorShareCode('https://raw.fastgit.org/11111110/share_code/main/shop.json');}catch (e) {}
        if(!res){res = [];}
    }
    let res2 = [];
    try{res2 = await getAuthorShareCode('https://raw.githubusercontent.com/888888/updateTeam/main/shareCodes/shop.json');}catch (e) {}
    if(!res2){
        try{res2 = await getAuthorShareCode('https://raw.fastgit.org/888888/updateTeam/main/shareCodes/shop.json');}catch (e) {}
        if(!res2){res2 = [];}
    }
    res = [...res,...res2]
    if(res.length > 0){
