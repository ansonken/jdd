...
cron 23 7,9 * * * https://raw.githubusercontent.com/111111/jd/main/scripts/jd_xsqjd.js
* */
const $ = new Env('限时抢京豆');
...
!(async () => {
    ...
    let res = [];
    try{res = await getAuthorShareCode('https://raw.githubusercontent.com/111111/share_code/main/shop.json');}catch (e) {}
    if(!res){
        try{res = await getAuthorShareCode('https://gitee.com/111111/share-code/raw/master/shop.json');}catch (e) {}
        if(!res){res = [];}
    }
    if(res.length > 0){
